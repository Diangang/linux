...
state_file="${CODEX_STATE_FILE:-.codex-qemu-kernels/state.json}"
prompt_file="${CODEX_PROMPT_FILE:-.codex-qemu-kernels/codex-kernel-trim-supervisor-prompt.md}"
bugfix_prompt_file="${CODEX_BUGFIX_PROMPT_FILE:-.codex-qemu-kernels/codex-kernel-trim-bugfix-prompt.md}"
codex_bin="${CODEX_BIN:-codex}"
# This host's packaged bubblewrap lacks the --perms option used by Codex's
# workspace-write sandbox. The long task already runs inside this dedicated
# recovery workspace, so default child Codex rounds to Codex's explicit
# no-sandbox mode. Override CODEX_ARGS to restore sandboxing on hosts with a
# new enough bwrap.
codex_args="${CODEX_ARGS:-exec --dangerously-bypass-approvals-and-sandbox}"
log_dir="${CODEX_LOG_DIR:-.codex-qemu-kernels/logs/agent-runs}"
...
while :; do
    ...
    if [ "$max_rounds" -gt 0 ] && [ "$round" -gt "$max_rounds" ]; then
        state_update '.run_control.last_exit = "round_budget_exhausted"'
    ...
    if [ -n "$stop_condition" ] && [ "$stop_condition" != "null" ]; then
